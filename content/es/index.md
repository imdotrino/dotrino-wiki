---
title: Dotrino Wiki
description: La documentación del ecosistema Dotrino, en un solo lugar: la bóveda, los pilares, las apps y cómo contribuir.
---

# Dotrino Wiki

La documentación del ecosistema [Dotrino](https://dotrino.com) en un solo lugar.
Dotrino es un ecosistema de aplicaciones centradas en la privacidad de tus datos:
**lo tuyo es tuyo, y tú decides qué compartes, con quién y cuándo.**

## Por dónde empezar

- [Qué es Dotrino](/empezar/que-es/) — el ecosistema en dos minutos.
- [Instalar una app en tu teléfono](/empezar/instalar-apps/) y [Tu identidad y tus perfiles](/empezar/identidad/).
- [Cómo se protege lo tuyo](/empezar/privacidad/) y [Cómo viaja lo que compartes](/empezar/como-viaja/).
- [La bóveda](/vault/modelo/) — tu certificador y tu almacén, en tu propia máquina.
- [Instalarla](/vault/instalacion/) y [conectar tus aparatos](/vault/emparejar/).
- [Secretos de servicios](/vault/secretos/): que tus programas dejen de llevar un `.env`.
- [Aprobación desde el teléfono](/vault/aprobacion/) y [llaves SSH sin archivos](/vault/ssh/).

## Las aplicaciones

- [Chat y Messenger](/apps/mensajeria/), [Eco](/apps/eco/), [here](/apps/here/) y [Trueque](/apps/trueque/) — hablar, publicar y encontrarse.
- [Perfil y calificaciones](/apps/perfil/), [Wallet](/apps/wallet/) y [QRShare](/apps/compartir-archivos/).
- [Los juegos](/apps/juegos/), [Deporte](/apps/deporte/), [Utilidades](/apps/utilidades/), [códigos QR](/apps/qr/) y [Android Launcher](/apps/android/).

## Herramientas para tu máquina

- [El instalador](/herramientas/instalar/) — un comando y listo.
- [Inspector](/herramientas/inspector/) — qué contraseñas tienes a la vista, y qué hacer.
- [Terminal](/herramientas/terminal/), [asistentes de IA](/herramientas/ia/) y [el túnel](/herramientas/tunel/).

## Para desarrollar

- [Los pilares](/desarrollo/pilares/) y [los componentes de interfaz](/desarrollo/componentes/).
- [Los servicios](/desarrollo/servicios/) y [el índice y las pruebas](/desarrollo/indice/).

## Para una empresa

- [Qué es Dotrino Enterprise](/empresa/que-es/) — la misma arquitectura, para tu empresa.
- [Cómo empezar en tu empresa](/empresa/como-empezar/) — los cuatro pasos, en orden.

## Cómo se edita este wiki

Cada página tiene abajo un botón **«Editar en GitHub»**: el contenido vive en
Markdown en el repositorio [`dotrino-wiki`](https://github.com/imdotrino/dotrino-wiki)
y se publica estático con un generador propio — sin cuentas, sin cookies, sin
JavaScript de terceros. Los cambios entran por *pull request* a la rama `develop`.
